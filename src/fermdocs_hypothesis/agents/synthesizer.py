...
SYNTHESIZER_INVARIANTS = (
    ...
    "TOOL GAP vs DATA GAP — the [SYMMETRY] distinction: when a finding's"
    " statistics include `symmetry_violation: True`, the absence of that"
    " metric on that run is a TOOL gap (the data IS in the bundle, the"
    " toolchain failed to compute), NOT a DATA gap (data missing from"
    " the bundle). Phrase findings carefully: 'metric X could not be"
    " computed for run Y due to a known toolchain limitation' rather"
    " than 'run Y did not have data for metric X'. Crucially, do NOT"
    " let a TOOL gap block a comparative conclusion supported by"
    " metrics that DID compute on all runs. The honest answer to"
    " 'which run was better?' is the conclusion drawn from the"
    ...
    " h0_outlier, scale_change, bioreactor_change}) AND your hypothesis"
    " makes a cross-batch comparison touching the affected runs or"
    " variables, you MUST acknowledge the confound in the summary."
    " Example: 'WCW comparisons across RUN-3 (Hitachi) vs RUN-4 (LABMAN)"
    " are flagged for instrument bias [F-NNNN]; the trend is consistent"
    " with the calibration offset and not necessarily a biological"
    " effect.' Either cite the metadata-anomaly finding and explain why"
    " the conclusion is robust to the confound, or downgrade the claim"
    " to within-instrument / within-scale comparisons. Silently"
    " comparing across the confound is a rejection.",
    ...
    " 'over time', 'after Nh', 'before peak', 'post-induction' — you"
    " MUST populate cited_trajectories with ≥1 entry naming the"
    " (run_id, variable) the dynamic claim is grounded in. Trajectories"
    " are the highest-information signal in the bundle; relying only on"
    " point-in-time findings to assert dynamics is the gap that lets"
    " 'data shows X over time' hypotheses ship with zero trajectory"
    " citations. If view.relevant_trajectories is empty for the (run,"
    " variable) you'd cite, downgrade the claim from dynamic to"
    " point-in-time ('observed at h=144' rather than 'declines after"
    " 144h'). Don't fabricate a trajectory citation; cite the right"
    " trajectory or weaken the claim.",
